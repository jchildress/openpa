...
public class SpSymFltMatrix implements FloatMatrix {
    ...
    protected void setAdjacencies(LinkNet adj) {
        _adj = adj;
    }

    protected void setBOffDiag(float[] boffdiag) {
        _boffdiag = boffdiag;
    }
     ...
     *
     * @param bdiag    diagonal susceptance values (array with size equal to number
     *                 of buses)
     * @param boffdiag off-diagonal susceptance values (array with size equal to
     *                 number of branches)
    ...
    protected void setBDiag(float[] bdiag) {
        _bdiag = bdiag;
    }
