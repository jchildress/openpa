...
//				}
    ...
    public void setCovergenceTolerance(float tol) {
        _cnvtol = PAMath.mva2pu(tol, _sbase);
    }
     ...
     */
    public float getConvergenceTolerance() {
        return PAMath.pu2mva(_cnvtol, _sbase);
    }
     ...
     * within this value, then slack is distributed for that iteration.  Defaults to 10
     *
     * @param tol Slack activation tolerance in MW
     */
     ...
     */
    public float getUnitActiveLimitTolerance() {
        return _pdisttol * 100f;
    }
     ...
     * distribution up to this level. Defaults to 100%
     *
     * @param tol percent of unit limit.
     */
    ...
    @Deprecated
    public BusTypeUtil getBusTypes() {
        return _btu;
    }
    ...
    abstract static class VarAllocator {
        ...
        static VarAllocator Create(SVC g, float m) {
            return (m < 0f) ? (new VarAllocator(g) {
                @Override
                public float getQLim() throws PAModelException {
                    return g.getMinQ();
                }
            }) : (new VarAllocator(g) {
                @Override
                float getQLim() throws PAModelException {
                    return g.getMaxQ();
                }
    ...
    class DistInfo {
        ...
        class PosMMDistributor extends Distributor {
            PosMMDistributor(float mm) {
                super(mm);
            }

            @Override
            boolean testRemaining() {
                return (_rem > _cnvtol);
            }

            @Override
            boolean testAvail(float pdes, float pavail) {
                return (pavail > pdes);
            }

            @Override
            float calcAvail(float pdes, float pemer, float ps) {
                return Math.min(pdes, pemer - ps);
            }
        }

        class NegMMDistributor extends Distributor {
            NegMMDistributor(float mm) {
                super(mm);
            }
