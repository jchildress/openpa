...
//			Pattern.compile("\\s*,*[\\s/]+");
    ...
    public PsseHeader(LineNumberReader rdr) throws IOException {
        ...
        for (int i = end; i < l1.length(); ++i, ++end) {
            char c = l1.charAt(i);
            if (c != Character.LINE_SEPARATOR && c != '/' && c != ' ' && c != ',') sb.append(c);
            else break;
        }
    ...
    public Float getSystemBaseMVA() {
        return _systemBaseMVA;
    }

    public GregorianCalendar getCaseTime() {
        return _caseTime;
    }

    public GregorianCalendar getImportTime() {
        return _importTime;
    }
