...
public class PsseRepository {
    ...
    Consumer<PrintWriter> _cleaner = i -> {
        i.flush();
        i.close();
    };
    private File _dir;
    ...
    public PrintWriter findWriter(PsmFormat f) {
        return _wrtrmap.computeIfAbsent(f, _psmwrtr);
    }

    public PrintWriter findWriter(CaseFormat f) {
        return _casewmap.computeIfAbsent(f, _casewrtr);
    }
