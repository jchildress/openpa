...
import com.powerdata.openpa.impl.*;
...
//	SimpleCSV _relayOperateCSV;
    ...
    ublic PFlowPsmModelBldr(String parms) throws PAModelException {
    ...
    ublic PFlowPsmModelBldr(File dir) {
        _dir = dir;
    }
    ...
    @
            oid
    testLoadTransformerCSVs() throws IOException {
    ...
    @
            ublic
    int[] remapIndex(int[] inner, int[] outer) {
        int n = inner.length;
        int[] rv = new int[n];
        for (int i = 0; i < n; ++i)
            rv[i] = outer[inner[i]];
    ...
    protected <R> R load(ListMetaType ltype, ColumnMeta ctype, int[] keys)
        ...
        switch (ctype) {
            ...
            case PhashHASREG:
                return (R) _ps.hasReg();
            case PhashMXANG:
                return (R) _ps.getMaxAng();
            case PhashMNANG:
                return (R) _ps.getMinAng();
            case PhashMXMW:
                return (R) _ps.getMaxMW();
            case PhashMNMW:
                return (R) _ps.getMinMW();
            //Transformer
            case TfmrID:
            ...
            case TfmrHASREG:
                return (R) _xf.hasReg();
            case TfmrTAPBUS:
                return (R) _xf.getTapBus();
            case TfmrREGENAB:
                return (R) _xf.isRegEnabl();
            case TfmrMINREGKV:
                return (R) _xf.getMinKV();
            case TfmrMAXREGKV:
                return (R) _xf.getMaxKV();
            case TfmrMNTPFROM:
                return (R) _xf.getFromMinTap();
            case TfmrMXTPFROM:
                return (R) _xf.getFromMaxTap();
            case TfmrMNTPTO:
                return (R) _xf.getToMinTap();
            case TfmrMXTPTO:
                return (R) _xf.getToMaxTap();
            case TfmrSTEPFROM:
                return (R) _xf.getFromStep();
            case TfmrSTEPTO:
                return (R) _xf.getToStep();
            case SwID:
    ...
    oolean[] resolveOOS(int[] idx, String[] data) {
        if (data.length == 0) {
    ...
    @
            rivate
    float[] invertValues(float[] origData) {
    ...
    oolean[] getBooleanData(int[] idx, String[] data) {
    ...
    oolean[] getBooleanData(int[] idx, String[] data, boolean deft) {
        if (data.length == 0) return new boolean[0];
        int n = idx.length;
        boolean[] rv = new boolean[n];
        for (int i = 0; i < n; ++i) {
...
//
    ...
    boolean[] getGenAVRMode() throws PAModelException {
    ...
    int[] getGenRegBus() throws PAModelException {
...
**
    ...
    //
    loat[] getFloatData(int[] idx, String[] data, float deft) {
        int nidx = idx.length;
    ...
    nt[] getIntData(int[] idx, String[] data, int deft) {
        int nidx = idx.length;
        int[] rv = new int[nidx];
        Arrays.fill(rv, deft);
        int ndata = data.length;
    ...
    tring[] getStringData(int[] idx, String[] data) {
        int n = idx.length;
...
//						inv = ttapxInv;
        ...
        public int[] getWindingIndex() {
            return _wndx;
        }

        public int[] getTransformerIndex() {
            return _wtx;
        }

        public int[] getWindingCaseIndex() {
            return _wcasex;
        }
    ...
    f
    lass PsWindPart extends WndPart2
    ...
    i
    lass TxWindPart extends WndPart2
...
class IndexBuilder {
     ...
     *
     * @param source  The source object. Entries in the source object are hashed
     *                for subsequent reference builders.
     * @param linkcol The source column used to create linkages to related
     *                objects
     */
